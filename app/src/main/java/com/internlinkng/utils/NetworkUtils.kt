...
object NetworkUtils {
    ...
    suspend fun getHospitals(): List<Hospital> = withContext(Dispatchers.IO) {
        try {
            ...
            val response = SupabaseClient.getDatabase()
                .from("hospitals")
                .select()
                .decodeList<Hospital>()
    ...
    suspend fun login(email: String, password: String): AuthResult {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val response = SupabaseClient.getAuth().signInWith(email, password)
    ...
    suspend fun signUp(email: String, password: String): AuthResult {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val response = SupabaseClient.getAuth().signUpWith(email, password)
    ...
    suspend fun logout(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                SupabaseClient.getAuth().signOut()
    ...
    suspend fun getCurrentUser(): io.github.jan.supabase.gotrue.user.UserInfo? {
        return withContext(Dispatchers.IO) {
            try {
                SupabaseClient.getAuth().currentUserOrNull()
            } catch (e: Exception) {
    ...
    fun isLoggedIn(): Boolean {
        return try {
            SupabaseClient.getAuth().currentUserOrNull() != null
        } catch (e: Exception) {
